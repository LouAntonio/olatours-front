...
import { useUsers, useCreateUser, useDeleteUser } from '../../hooks/useUsers.ts';
...
export function AdminUsers() {
	...
	return (
		<div>
			<div className="flex items-center justify-between mb-8">
				<div>
					...
					<p className="mt-1 text-ink-mute text-sm">
						{users ? `${users.length} administrador(es)` : 'Gerir administradores'}
					</p>
				</div>
				<button
					type="button"
			...
			{showForm && (
				<form onSubmit={handleCreate} className="mb-8 bg-white border border-gray-border/60 rounded-lg p-5 space-y-4">
					<h2 className="label-caps text-ink-mute text-xs uppercase tracking-wider">Novo Administrador</h2>

					{formError && (
						<div className="bg-red-50 border border-red-200 rounded-sm px-4 py-3 text-red-700 text-sm">{formError}</div>
					)}
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						...
						<div>
							<label className="label-caps text-ink-mute text-xs block mb-1">Palavra-passe *</label>
			...
			{isLoading ? (
					...
					<table className="w-full text-sm">
						<thead>
							<tr className="border-b border-gray-border/60 bg-cream-50">
								<th className="text-left px-4 py-3 label-caps text-ink-mute text-[10px]">Nome</th>
								<th className="text-left px-4 py-3 label-caps text-ink-mute text-[10px] hidden sm:table-cell">Email</th>
								<th className="text-left px-4 py-3 label-caps text-ink-mute text-[10px] hidden md:table-cell">Registo</th>
								<th className="text-right px-4 py-3 label-caps text-ink-mute text-[10px]">Ações</th>
							</tr>
						</thead>
						<tbody>
							{users.map((user) => (
								<tr key={user.id} className="border-b border-gray-border/30 hover:bg-cream-50/50 transition-colors">
									<td className="px-4 py-3.5">
										<p className="font-semibold text-ink">{user.name} {user.surname}</p>
									</td>
									<td className="px-4 py-3.5 text-ink-mute hidden sm:table-cell">{user.email}</td>
									...
									<td className="px-4 py-3.5 text-right">
										...
										{currentUser?.id === user.id && (
											<span className="label-caps text-[10px] text-ink-mute">Você</span>
										)}
									</td>
								</tr>
							))}
				...
				<div className="bg-white border border-gray-border/60 rounded-lg p-10 text-center">
					<p className="text-ink-mute text-sm">Nenhum utilizador encontrado.</p>
				</div>
			)}
		</div>
	);
