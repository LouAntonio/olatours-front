...
import { useEvent, createEvent, updateEvent } from '../../hooks/useEvents.ts';
	...
	return (
		<div className="max-w-2xl">
			...
			<form onSubmit={handleSubmit} className="space-y-5">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						...
						<div>
							...
							<input
								...
								onChange={(e) => update('displayDateLong', e.target.value)}
								className="w-full border border-gray-border/60 rounded-sm px-3 py-2 text-sm text-ink focus:outline-none focus:border-flag transition-colors"
				...
				<div className="bg-white border border-gray-border/60 rounded-lg p-5 space-y-4">
					<h2 className="label-caps text-ink-mute text-xs uppercase tracking-wider">Imagem</h2>
					<div>
						<label className="label-caps text-ink-mute text-xs block mb-1">URL da Imagem de Capa</label>
						<input
							value={form.coverImage}
							onChange={(e) => update('coverImage', e.target.value)}
							className="w-full border border-gray-border/60 rounded-sm px-3 py-2 text-sm text-ink focus:outline-none focus:border-flag transition-colors"
							placeholder="/uploads/events/... ou URL externa"
						/>
					</div>
				</div>
