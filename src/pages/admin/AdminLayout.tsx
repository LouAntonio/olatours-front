...
export function AdminLayout() {
	...
	const { user, isAuthenticated, isInitialized, logout, hydrate } = useAuthStore();
		...
		<div className="min-h-dvh bg-cream-50 flex">
			...
			>
				<div className="flex items-center justify-between h-16 px-5 border-b border-white/10 shrink-0">
					<Logo size="sm" className="[filter:brightness(0)_invert(1)]" />
					<button type="button" onClick={() => setSidebarOpen(false)}
						className="lg:hidden text-white/60 hover:text-white" aria-label="Fechar menu">
						...
						<NavLink key={item.path} to={item.path} end={item.end} onClick={() => setSidebarOpen(false)}
							className={({ isActive }) =>
								`flex items-center gap-3 px-3 py-2.5 rounded-sm text-sm label-caps transition-colors ${
									isActive ? 'bg-flag/20 text-flag' : 'text-white/60 hover:text-white hover:bg-white/5'
								}`
							}
						>
							{item.label}
				...
				<div className="border-t border-white/10 px-3 py-4 shrink-0">
					<div className="px-3 mb-2">
						<p className="text-white/80 text-sm font-semibold truncate">{user?.name} {user?.surname}</p>
						<p className="text-white/40 text-xs truncate">{user?.email}</p>
					</div>
					<NavLink to="/ot/perfil" onClick={() => setSidebarOpen(false)}
						className={({ isActive }) =>
							`flex items-center gap-3 px-3 py-2.5 rounded-sm text-sm label-caps transition-colors ${
								isActive ? 'bg-flag/20 text-flag' : 'text-white/40 hover:text-white hover:bg-white/5'
							}`
						}
					>
						<svg viewBox="0 0 16 16" fill="none" className="h-3.5 w-3.5">
							<path d="M8 8a3 3 0 100-6 3 3 0 000 6zm-5 6a5 5 0 0110 0" stroke="currentColor" strokeWidth="1.5" />
						</svg>
						Perfil
					</NavLink>
					...
					>
						<svg viewBox="0 0 16 16" fill="none" className="h-3.5 w-3.5">
							<path d="M6 14H3a1 1 0 01-1-1V3a1 1 0 011-1h3M11 11l3-3-3-3M14 8H6" stroke="currentColor" strokeWidth="1.5" />
						</svg>
						Sair
					</button>
				</div>
			...
			<div className="flex-1 flex flex-col min-w-0">
				<header className="h-16 bg-white border-b border-gray-border/60 flex items-center px-5 shrink-0">
					<button type="button" onClick={() => setSidebarOpen(true)}
						className="lg:hidden mr-3 text-ink hover:text-flag transition-colors" aria-label="Abrir menu">
